#!/usr/bin/env python3
"""Generate Axinom ingest Excel template v1.3.0 with validation and formatting."""
...
OUTPUT_PATH = ROOT / "docs" / "reference" / "axinom_ingest_template_v1_3_0.xlsx"
...
HEADERS = [
    ("Asset Type", 15.0, "Required. One of MOVIE, TVSHOW, SEASON, EPISODE."),
    ("External ID", 78.0, "Required for all asset types."),
    ("Title", 54.33203125, "Required for MOVIE, TVSHOW, and EPISODE."),
    ...
    ("Parent External ID", 74.1640625, "Required for SEASON and EPISODE."),
    ...
    ("Video Source", 62.1640625, "Required for MOVIE and EPISODE in minimum ingest flow."),
    ("Video Profile", 20.0, "Required for MOVIE and EPISODE in minimum ingest flow."),
    ("Trailer Source", 62.1640625, "Required when Trailer Profile is set."),
...
EXAMPLE_ROWS = [
    {
        ...
        "Video Profile": "DRM (HLS+Dash) HD",
        "Trailer Source": "trailers/example_movie_t1",
        "Trailer Profile": "nDRM (HLS-Only) HD",
    ...
    },
    {
        ...
        "Video Profile": "nDRM (HLS-Only) HD",
...
def main() -> None:
    ...
    # Match legacy workbook look with Poppins body font and taller example rows.
    ...
    add_required_blank_cf(ws, "Title", '=AND(OR($A2="MOVIE",$A2="TVSHOW",$A2="EPISODE"),LEN(TRIM($C2))=0)')
    add_required_blank_cf(ws, "Season/Ep Number", '=AND(OR($A2="SEASON",$A2="EPISODE"),LEN(TRIM($I2))=0)')
    add_required_blank_cf(ws, "Parent External ID", '=AND(OR($A2="SEASON",$A2="EPISODE"),LEN(TRIM($J2))=0)')
    add_required_blank_cf(
        ws,
        "Video Source",
        '=AND(OR($A2="MOVIE",$A2="EPISODE",LEN(TRIM($S2))>0),LEN(TRIM($R2))=0)',
    )
    add_required_blank_cf(
        ws,
        "Video Profile",
        '=AND(OR($A2="MOVIE",$A2="EPISODE",LEN(TRIM($R2))>0),LEN(TRIM($S2))=0)',
    )
    add_required_blank_cf(ws, "Trailer Source", '=AND(LEN(TRIM($U2))>0,LEN(TRIM($T2))=0)')
